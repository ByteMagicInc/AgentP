...
2. Commit the notes and start from a clean, up-to-date `master`. CI (fmt, clippy, tests) runs automatically only as part of a release, where it gates the build; to check before tagging, run it from **Actions → CI → Run workflow**, or locally with `cargo fmt --all -- --check`, `cargo clippy --all-targets --locked -- -D warnings`, and `cargo test --all --locked`.
